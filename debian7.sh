...
sed -i 's|export KEY_PROVINCE="CA"|export KEY_PROVINCE="BANTENt"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_CITY="SanFrancisco"|export KEY_CITY="PANDEGLANG"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_ORG="Fort-Funston"|export KEY_ORG="KANG WAHID"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_EMAIL="kangwahid@script.com"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_EMAIL=mail@host.domain|export KEY_EMAIL=kangwahid@script.com|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_CN=changeme|export KEY_CN="kangwahid.script.com"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_NAME=changeme|export KEY_NAME=kangwahid.script.com|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_OU=changeme|export KEY_OU=KANGWAHID|' /etc/openvpn/easy-rsa/2.0/vars
# Buat PKI
...
echo "                         Copyright Script grtais.net                            "
echo "                                   "Kang WaHid"
echo "                        Created By Kang Wahid(fb.com/kangbae.1)                 "
echo "--------------------------------------------------------------------------------"
...
echo "     Silahkan Restart VPS anda......  TERIMA KASIH"  | tee -a log-install.txt
echo ""  | tee -a log-install.txt
